...
export const ECOMMERCE_IMAGE_SET_CORE_PROMPT = [
  ...
  '先根据用户确认的补充说明判断本轮需要生成的图片数量和类型；如果用户明确指定数量或范围，必须遵守用户指定；如果用户没有明确指定数量，则由模型根据素材完整度、必卖理由和电商表达需要自行决定 prompts 数组数量。',
  ...
  '根据用户确认补充说明和产品需求选择模块组合；用户明确指定详情图数量时按用户数量输出，用户未指定时由模型自行判断是否生成详情页图以及生成多少张。每个模块只写三样：画面 / 图内文案 / 设计说明。',
...
export const ECOMMERCE_IMAGE_SET_SYSTEM_PROMPT = [
  ...
  '- Step 3 的图片数量由用户确认补充说明优先决定；如果用户没有明确指定数量，则由模型根据当前材料和策略自行决定输出多少条 prompt。',
  '- Step 4 详情页模块按需取舍，M5/M8 可选；M7 最后一题必须是产品属性界定+免责声明。',
  '- 每张图的 prompt 必须完整、可独立用于生图，包含主体、构图、背景、光线、材质、风格、文字区域、画面/文案/设计说明和电商用途。',
  '- prompts 数组只包含本轮需要实际送入画廊生成的图片；每条 prompt 对应 1 个画廊生图任务。',
  '- Step 5 自审评分总分必须 >= 80；低于 80 时自行重写后再输出。',
  '',
  '# 输出格式',
  '只输出 JSON，不要输出 Markdown、解释或代码块。格式：',
  '{"analysis":"策略分析","artifactMarkdown":"完整 Markdown 执行方案，包含 isArtifact=true 标记、Step 3 主图方案、Step 4 详情页模块、Step 5 自审评分、Step 6 输出","selfReviewScore":90,"prompts":[{"title":"图1 首图・点击率核心","purpose":"主图/详情页用途","prompt":"完整生图提示词","size":"1024x1024","inputImageIndexes":[0]},{"title":"详情1 首屏痛点共鸣","purpose":"详情页模块","prompt":"完整生图提示词","size":"1024x1024","inputImageIndexes":[0]}]}',
].join('\n')
